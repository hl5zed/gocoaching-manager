...
function logMonthlyDeleteFailure(
  reason: string,
  metadata: {
  ...
  console.error("[MONTHLY_REFLECTION_DELETE_FAILED]", {
    reason,
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const result = await updateMonthlyReflection(id, input);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  if (adminClientError || !adminClient) {
    logMonthlyDeleteFailure("admin client unavailable", {
      id,
  ...
  const { data: monthlyReflection, error: lookupError } = await admin
  ...
  if (lookupError) {
    logMonthlyDeleteFailure("monthly reflection lookup failed", {
  ...
  if (!monthlyReflection || monthlyReflection.deleted_at) {
    return jsonResponse(
      {
        ok: false,
  ...
  if (monthlyReflection.profile_id !== currentProfile.id) {
    return jsonResponse(
      {
        ok: false,
        message: "이 월간 회고에 접근할 권한이 없습니다.",
      },
      403,
    );
  }
  ...
  if (updateError) {
    logMonthlyDeleteFailure("monthly reflection soft delete failed", {
      currentProfileId: currentProfile.id,
      id,
